...
def _validate_webhook_url(url: str) -> None:
    ...
    try:
        ...
        raise NotificationError(f"Invalid webhook URL: {e}") from e
...
def _send_http_request(
    ...
    try:
        ...
        raise NotificationError(f"Failed to serialize payload: {e}") from e
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            ...
            return {
                "status_code": e.code,
                "body": (
                    e.read().decode("utf-8", errors="replace")[:1000] if e.fp else ""
                ),
                "success": False,
                "error": str(e),
            }
    ...
    raise NotificationError(
        f"Failed to send notification after {MAX_RETRIES + 1} attempts: {last_error}"
    )
    ...
    if is_slack:
        ...
        fields = [
            {"title": "Industry", "value": industry, "short": True},
            {
                ...
                "value": str(metrics_analyzed),
                "short": True,
            },
            {
                "title": "Health Status",
                "value": _sanitize_text(overall_health, 50),
                "short": True,
            },
